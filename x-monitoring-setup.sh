...
if [[ -z "${URLS[0]}" ]]; then
  echo "Mindestens eine URL muss gesetzt sein." >&2
  exit 1
fi
for d in "${DURATIONS[@]}"; do
  if ! is_positive_int "$d"; then
    echo "Alle Dauerwerte müssen positive ganze Zahlen sein." >&2
    exit 1
  fi
done
#Pakete
...
#rotation
DASHBOARD_START_URL="${URLS[0]}"
if (( ${#URLS[@]} > 1 )); then
  ROTATOR_HTML="/home/${KIOSK_USER}/prtg-rotator.html"
  URL_JS=$(printf '"%s",' "${URLS[@]}")
  URL_JS="[${URL_JS%,}]"
  DUR_JS=$(printf '%s000,' "${DURATIONS[@]}")
  DUR_JS="[${DUR_JS%,}]"
...
<body>
<iframe id="frame" src="${URLS[0]}"></iframe>
<script>
  const urls = ${URL_JS};
  const durations = ${DUR_JS};
  let idx = 0;
...
if [[ -f /etc/lightdm/lightdm.conf ]]; then
  if grep -q '^autologin-user=' /etc/lightdm/lightdm.conf; then
    ...
    sed -i "/^
\[Seat:\*\]
/a autologin-user=${KIOSK_USER}" /etc/lightdm/lightdm.conf || \
